...
- **Portal de Pagos B2B:** Incluye un flow público `/pagos` interactivo y un `/pagos/:id` simulando pasarelas (Mercado Pago, PayPal) para que clientes abonen servicios B2B.
- **Backend (Python/Render):** FastAPI, SQLAlchemy, SQLite (fallback local) o Postgres. Aún conserva la lógica dormida de e-commerce transaccional (stock y orders) por si a futuro la agencia decide vender servicios automatizados limitados sin intervención humana.
- **Arquitectura de Mocks (Frontend-First para Porfolio):** Se implementó una gestión estática y dinámica "Mocked" a través de `news.ts`, `useSolutionsAdmin.ts` y `useInvoicesAdmin.ts` (apoyado en `localStorage`). Esto permite que el CRUD de Novedades, Soluciones, Facturas y **Métricas Financieras del Dashboard** ocurra 100% en el frontend (Vercel).
  - *¿Por qué se tomó esta decisión?* Porque el proyecto es un MVP/Portfolio demostrativo B2B puro. Al tener lógica financiera y CRUD en frontend, garantizamos que el "Admin Panel" funcione hiper rápido 24/7 de cara al reclutador o profesor, sin sufrir las congelaciones por inactividad ("cold starts") típicas o caídas de las Bases de Datos Gratuitas de PostgreSQL en Render.
- **Panel Administrativo (SaaS Dashboard):** Cuenta con un panel de gestión en `midominio/admin/login` (contraseña demo `admin/admin`). Protege rutas como `/admin/dashboard`, `/admin/novedades`, `/admin/soluciones` y `/admin/invoices`. El Dashboard posee Widgets Financieros (Revenue Mensual, Anual y Crecimiento en tiempo real) que se autocalculan leyendo las facturas B2B pagadas almacenadas en la memoria del navegador.
